...
while getopts 'hfno-:' opt; do
    case "$opt" in
        ...
        *) echo "Error: the only valid options are --dry-run, -n, --force, -f, --help, -h, --overwrite, -o" >&2
           exit 1
    esac
done

if [[ $display_help == true ]]; then
  echo "usage: $0 -h, --help, -f, --force, -n, --dry-run, -o, --overwrite"
  exit 2
fi
...
rawjson=`curl -s https://plex.tv/api/downloads/5.json?channel=plexpass`
